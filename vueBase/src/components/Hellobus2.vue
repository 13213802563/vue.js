		      ...
		      <tr v-for="(data,index) in item">
		        ...
		        <td>
		          <div class="quantity-selector clearfix">
		            ...
		            <input type="number" v-bind:value="data.num" class="number" v-bind:data-realStock="data.realStock" v-on:keyUp="numEntry(index1, $index)" v-on:keyDown="numEntry(index1, $index)"/>
		            <span class="add" v-on:click="numChange(index1, $index, 1)" v-bind:class="{ 'disable' : data.num==data.realStock }">＋</span>-->
		            <span class="reduce" v-model='index'  v-on:click="counter += 1" v-bind:class="{ 'disable' : data.num==1 }">{{index}}</span>
		            <input type="number" v-bind:value="data.num" class="number" v-bind:data-realStock="data.realStock" v-on:keyUp="numEntry(index1, $index)" v-on:keyDown="numEntry(index1, $index)"/>
		            <span class="add" v-on:click="numChange(index1, $index, 1)" v-bind:class="{ 'disable' : data.num==data.realStock }">＋</span>
		          </div>
		        </td>
