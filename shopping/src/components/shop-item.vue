     ...
     <!--坚果 Pro 软胶保护套-->
      ...
      <div class="params-colors">
        <ul class="colors-list">
        	<!--按钮的颜色(v-for遍历  ，key定义index为索引）-->
          <li key="index" v-for="sku,index in item.sku_info">
          <!--点击更换背景颜色   @click="tableData(index)"  title=“红色”   data-index动态获取索引值--><!--index为当前点击索引，itemIndex为信息库索引-->
...
//  	定义获取信息的索引值  
  ...
  methods: {
    ...
    //点击背影颜色选择按钮执行的方法，点击索引为index，即当前背景索引为itemIndex
    tableData (index) {
